...
class Rejected(Exception):
    """A stable, redaction-safe verification failure."""
    ...
    try:
        ...
        value = json.loads(
            ...
            parse_constant=lambda _value: (_ for _ in ()).throw(
                Rejected("maintainer_release_metadata_invalid")
            ),
        )
    except Rejected:
        raise
...
def repository_coordinates(repository_id: str) -> dict[str, str]:
    if repository_id == "javascript":
        ...
        require_package(
            ROOT / "packages/vercel-ai/package.json", "@latchway/vercel-ai"
        )
        require_package(
            ROOT / "packages/langchain/package.json", "@latchway/langchain"
        )
        return {}
    if repository_id == "ios":
        try:
            swift = (ROOT / "Sources/Latchway/LatchwayVersion.swift").read_text(
                encoding="utf-8"
            )
        ...
        if (
            re.search(r'\bsdk\s*=\s*"1\.0\.0"', swift) is None
            or re.search(r"\b(?:s|spec)\.version\s*=\s*['\"]1\.0\.0['\"]", podspec) is None
        ):
            raise Rejected("maintainer_release_local_version_invalid")
        return {}
    ...
    try:
        ...
        javascript = lock["javascript"]
        ios = lock["ios"]
        android = lock["android"]
        react_native = lock["react_native"]
        expected = {
            "core_commit": contract["core_commit"],
            "contract_version": contract["version"],
            "javascript_commit": javascript["source_commit"],
            "javascript_version": javascript["version"],
            "ios_commit": ios["source_commit"],
            "ios_version": ios["version"],
            "android_commit": android["source_commit"],
            "android_version": android["version"],
            "react_native_version": react_native["version"],
        }
    except (KeyError, TypeError):
        raise Rejected("maintainer_release_compatibility_lock_invalid") from None
    if (
        contract.get("repository") != "https://github.com/Latchway/latchway.git"
        or javascript.get("package") != "@latchway/client"
        or javascript.get("repository")
        != "https://github.com/Latchway/latchway-js.git"
        or ios.get("pod") != "Latchway/AppAttest"
        or ios.get("repository")
        != "https://github.com/Latchway/latchway-ios-sdk.git"
        or android.get("group") != "dev.latchway"
        or android.get("repository")
        != "https://github.com/Latchway/latchway-android.git"
        or react_native.get("package") != "@latchway/react-native"
        or expected["contract_version"] != VERSION
        or expected["react_native_version"] != VERSION
        or any(
            COMMIT.fullmatch(expected[key]) is None
            for key in (
                "core_commit",
                "javascript_commit",
                "ios_commit",
...
def verify(arguments: argparse.Namespace) -> dict[str, str]:
    expected_directory, expected_repository = REPOSITORIES.get(
        arguments.repository_id, (None, None)
    )
    if (
        expected_directory is None
        or ROOT.name != expected_directory
        or arguments.repository_name != expected_repository
    ...
    intent = {
        ...
        "downstream_required_gates": [
            ...
            "complete_local_release_tests",
            "deterministic_package_archives",
            "registry_byte_verification",
            "trusted_publication_provenance",
